...
# 通用基础收入货币
现有的加密货币奖励机制通过工作量证明（POW）或权益证明（POS）将奖励分发给保护网络安全的矿工，但网络参与者并没有任何奖励。
现有的奖励机制始终会导致网络参与者之间非常不对称的奖励分配，因此UBIC提出了一种货币体系，所有网络参与者都将获得通用基础收入。

# 先前的研究和其他项目
截至2020年，互联网中存在数千种加密货币，UBIC并不是第一个旨在实现通用基础收入（UBI）发行的项目。非UBI和UBI加密货币之间
的主要区别在于解决女巫攻击。

一些类似 "Duniter" 的项目已经开始建立"信任网络"，并要求用户对彼此进行“验证”。 虽然这个想法看起来很简单的建立了“信任网络”，但
实际上这非常复杂，需要进行高级图论算法并进行手动验证。 另一种方法是针对参与者进行图灵测试 - 例如： “Idena” 使用翻转难题对参与
者进行验证，这将消耗参与者大量的时间和生命，并且对于能否抵抗人工智能学习依旧是一个长期的挑战。

UBIC尝试通过另一种方法来解决此问题，该方法利用信息差距较小的现代电子护照提供的近场通信（NFC）芯片进行验证。

# 经济体制
无限产出的货币或许并没有价值，这就是为什么每年产出的总量是固定的。
不同的货币运行在一个独特的区块链上，每种货币与一个国家/地区关联，并按照"U" + 国家/地区对应ISO 3166-2代码命名，例如：UCH 代表瑞士的货币， UDE 代表德国， UAT 代表奥地利 以及 UUK 代表英国。
区块产出很大一部分将平均分配给成功与护照关联的地址，以UCH为例，如果瑞士的UBIC货币每块产出100个并且有50本瑞士护照完成与地址关联，每本护照所关联的地址将获得2UCH。一段时间后，瑞士有500本完成与地址关联的护照，区块产出仍然是每块产出100个或多一点，每本护照所关联的地址将获得0.2UCH。

下列列表显示了每个国家/地区UBIC货币的发行数量。
...
| UMY	 | 马来西亚	                 | 38,684,160	 |  | 
| UUA	 | 乌克兰	                 | 54,767,520	 |  | 
| USP	 | 西班牙 	                 | 108,000,000	 | 2016年7月后的护照 |
| UHK	 | 香港	                 | 17,100,000	 | 2019年7月后的护照 | 
| UIS	 | 冰岛 	                 | 420,480	 | 2013年2月后的护照 |
| UEE	 | 爱沙尼亚	                 | 1,681,920	 | 尚无注册 | 
| UMC	 | 摩纳哥	                 | 52,560	 | 尚无注册 | 
| ULI	 | 列支敦士登	         | 52,560	 | 尚无注册 | 

# 防御女巫攻击
UBIC使用现代电子护照的安全功能抵抗女巫攻击. 您可以使用近场通信（NFC）读取现代电子护照加入UBIC. 得益于不可以转移的签名技术，
您的身份并不会因此泄漏，因为您的护照只有唯一的哈希值存储在区块链上，下一章节会进行更详细的解释。

现在，UBIC只能在28个国家/地区使用，这些国家已完全满足UBIC对电子护照的安全要求，以确保能够防御女巫攻击。一旦其他国家/地区
支持UBIC对电子护照的安全要求，UBIC可能会有更多的国家/地区的参与者加入。

# 电子护照
在过去的十年中，已经有许多国家引入了电子护照，有时我们也称之为生物特征护照。
这些护照通过近场通信（NFC）和加密算法提供了更强的完全性，几乎无法进行伪造。
电子护照是根据国际民用航空组织编写的护照9303标准制作，因此所有的电子护照都必须实现这些功能。
但由于每个国家/地区可以自由选择加密算法，其中一些国家/地区的加密强度还未达到UBIC的要求（例如：香港在2019年6月之前使用3位指
数的RSA加密，而UBIC需要指数大于65537位的RSA指数），这就是为什么不是所有的护照都能在UBIC中使用的原因。
...
### 文档签名证书
根据国际民用航空组织编写的护照9303标准规定，文档签名证书至少每三个月重新颁发一次，且签名时间有限，此后应当销毁其私钥。
这样可以确保如果一个文档签名证书被滥用，则只需要重新签发少量的护照，所有文档签名证书必须由国家签名证书颁发机构签名颁发。
 
### 国家签名证书
国家签名证书由国家签名证书颁发机构签名颁发，该证书在政府公钥基础设施中拥有最高级别的特权。
...
### 结构
交易可以由1个或多个又或是0个输入，1个或多个输出，输入值必须大于或等于输出值。
...
他们将由社区制定，然后由其他代表投票表决。如果有代表作恶，则其他代表应当撤票取消其代表权限。
...
### 政府敌意
政府可能对这种加密货币怀有敌意。它们可以尝试通过立法或黑客的手段攻击它。由于政府可以发放文档签名证书，它们可以通过注册大量护照滥用网络，但这似乎不太可能，因为这样会损害它们的信誉。
...
# 其他应用场景
### 了解你的客户（KYC）
虽然区块链本身并没有存储任何个人信息, 但用户可以决定将数据组1和数据组2中包含的信息透露给其他实体。
